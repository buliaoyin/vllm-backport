...
Status: planned implementation and validation completed on 2026-09-07. Confirmed code defects are fixed; documented model-output failures remain unresolved.
...
- GSM8K: official **test** split; 32-question initial compatibility screens for Qwen 27B and the first Radix baseline, 128-question GLM/Flash screens, and 1,319-question full comparisons for the stable GLM AWQ, GLM NVFP4 and Qwen Flash FP8 candidates. Five fixed training examples provide the prompt, using the repository's `_build_gsm8k_prompts` helper. Temperature 0, seed 42, request concurrency 4 for screening and 16 for full evaluation (the same for each full MTP0/3 pair). GLM uses its template with reasoning effort `max`, 4,096 output tokens; Qwen uses thinking enabled, 8,192 output tokens.
...
The following use all 1,319 official test questions at concurrency 16. Scores retain the official labels, including ambiguous or inconsistent items; diagnostic repetitions do not replace first-pass answers. All six full GSM8K evaluations are complete. The Qwen full cases retain failed long-key checks separately from their completed accuracy and functional results.
...
The same isolated 128-output-token code prompt supplies the aligned MTP0/MTP3 event profiles. MTP0 uses 127 recorded decode steps and MTP3 uses 41; per-step costs therefore are not costs per output token.
...
The full GSM8K results remain close with MTP0/3: NVFP4 −2 questions, AWQ +1, Qwen FP8 −1. Fixed-length decoding benefits depend on the prompt: 1.11×–1.65× across the two GLM formats, and 1.29×–2.05× for Qwen FP8. These are MTP-on/off measurements of the corrected repository, not a throughput A/B against the pre-fix source. Short compatibility and state checks additionally cover both Qwen Flash NVFP4 layouts and Qwen 27B BF16/FP8.

There are still observed output limitations: occasional GLM extra-role formatting, individual reasoning loops or interpretation changes (including the reproducible Radix MTP3 item-87 truncation), and the Qwen long public-key instruction failure. The Qwen control can fail without MTP, cache reuse or preceding user requests, and its behavior changes with batch/cache layout. This narrows the conditions but does not isolate all numerical paths or establish that a quantized checkpoint is intrinsically defective. No full BF16 GLM/Flash reference was run. The recorded wrong answers and assertion failures remain intact.
