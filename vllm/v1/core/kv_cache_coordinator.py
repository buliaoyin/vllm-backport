    ...
    def find_longest_cache_hit(
        ...
        while True:
            ...
            for idx, (spec, group_ids, manager_cls, use_eagle) in enumerate(
                ...
                # Match one replay margin past the candidate, then drop it.
                # Mamba needs a whole state interval even with fine-grained
                # hashing; other fine-grained managers drop one hash unit.
                if drop_eagle_block:
                    eagle_margin = (
                        self.hash_block_size
                        if self.enable_partial_hash_hits
